...
当前版本：**0.5.0-beta.19**。公开仓库默认运行隔离演示模式，不连接原项目的正式环境，不包含用户药品数据、照片或登录凭证。

beta.19 实施记录与发布前验收：[GitHub 参考功能交付记录](docs/beta19-reference-integration.md)。代码交付不代表微信正式版已发布。

## 功能

- 药盒与家庭成员管理，支持完整日期和月份精度的有效期。
- 开封期限、实际盘点、预计余量和固定服用计划，表单可预览未来 7 天安排。
- 存放位置筛选、独立新盒预填、同款药盒展示分组和可恢复的集中盘点。
